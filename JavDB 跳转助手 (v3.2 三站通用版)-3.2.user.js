// ==UserScript==
// @name         JavDB 跳转助手 (v3.2 三站通用版)
// @namespace    http://tampermonkey.net/
// @version      3.2
// @description  支持 MemoJAV, Jable.tv, SupJAV。非侵入式悬浮按钮，不崩溃、不漂移。
// @author       Assistant
...
(function() {
    ...
    // 优化的番号提取正则：支持 字母-数字, 字母数字（无横杠）, 以及长番号
    const getAvCode = (t) => {
        if (!t) return null;
        // 匹配常见的番号格式
    ...
    document.addEventListener('mouseover', function(e) {
        ...
        if (target) {
            ...
            // 尝试从不同的地方捞番号：标题链接、图片 alt、或者是元素文本
    ...
    // 3. 特殊适配：针对详情页按钮组的整合 (MemoJAV 原生风格)
